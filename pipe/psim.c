...
void do_fetch_stage()
{
    ...
    f_pc = memory_output->icode == I_JMP && !memory_output->takebranch ? 
                    memory_output -> vala : 
            writeback_output->icode == I_RET ? 
                    writeback_output -> valm 
            : fetch_output -> predPC;
    ...
    fetch_input->predPC = decode_input->icode == I_JMP || decode_input->icode == I_CALL ? 
                            decode_input -> valc : decode_input -> valp;

    /* TODO: Adjust this to account for error statuses */
    decode_input->status = imem_icode == I_HALT ? STAT_HLT : STAT_AOK;
...
{
    execute_input->srca = decode_output -> icode == I_RRMOVQ || 
                          decode_output -> icode == I_RMMOVQ || 
                          decode_output -> icode == I_ALU || 
                          decode_output -> icode == I_PUSHQ ?
                            decode_output-> ra
                          : decode_output -> icode == I_POPQ || 
                            decode_output -> icode == I_RET ?
                                REG_RSP
                          :     REG_NONE;

    execute_input -> srcb = decode_output -> icode == I_ALU || 
                            decode_output -> icode == I_RMMOVQ || 
                            decode_output -> icode == I_MRMOVQ || 
                            decode_output->icode == I_RRMOVQ ?
                                decode_output -> rb
                            : decode_output -> icode == I_PUSHQ ||
                              decode_output -> icode == I_POPQ ||
                              decode_output->icode == I_CALL || 
                              decode_output->icode == I_RET ?
                                REG_RSP
                            :   REG_NONE;

    execute_input -> deste = decode_output -> icode == I_RRMOVQ || decode_output -> icode == I_IRMOVQ || 
                             decode_output -> icode == I_ALU ?
                                decode_output -> rb
                            : decode_output -> icode == I_PUSHQ ||
                              decode_output->icode == I_CALL || 
    ...
    execute_input -> destm = decode_output -> icode == I_MRMOVQ || decode_output -> icode == I_POPQ ? decode_output -> ra : REG_NONE;

    execute_input->vala = decode_output->icode == I_CALL || decode_output->icode ==  I_JMP ? 
                                decode_output->valp
                            : execute_input->srca == memory_input->deste ? 
                                memory_input->vale
                            : execute_input->srca == memory_output->destm ? 
                                writeback_input->valm
                            : execute_input->srca == writeback_input->deste ? 
                                writeback_input->vale
                            : execute_input->srca == writeback_input->destm ? 
                                writeback_output->valm
                            : execute_input->srca == writeback_output->deste ? 
                                writeback_output->vale
                            :   get_reg_val(reg, execute_input->srca);
    
    execute_input->valb =   execute_input->srcb == memory_input->deste ? 
                                memory_input->vale
                            : execute_input->srcb == memory_output->destm ? 
                                writeback_input->valm
                            : execute_input->srcb == writeback_input->deste ? 
                                writeback_input->vale
                            : execute_input->srcb == writeback_input->destm ? 
                                writeback_output->valm
                            : execute_input->srcb == writeback_output->deste ? 
                                writeback_output->vale
                            :   get_reg_val(reg, execute_input->srcb);
...
void do_execute_stage()
{
     ...
     */
    if (execute_output->icode == I_ALU) setcc = true;

    alufun = execute_output -> icode == I_ALU ? execute_output -> ifun : A_ADD;

    alua = execute_output -> icode == I_RRMOVQ || 
           execute_output -> icode == I_ALU ? 
                execute_output -> vala 
           : execute_output -> icode == I_IRMOVQ || 
             execute_output -> icode == I_RMMOVQ || 
             execute_output -> icode == I_MRMOVQ ? 
                execute_output -> valc
           : execute_output -> icode == I_PUSHQ || execute_output -> icode == I_CALL ? -8
           : execute_output -> icode == I_POPQ || execute_output -> icode == I_RET ? 8
           : 0;

    alub = execute_output -> icode == I_RMMOVQ || 
           execute_output -> icode == I_MRMOVQ || 
           execute_output -> icode == I_ALU || 
           execute_output -> icode == I_CALL || 
           execute_output -> icode == I_PUSHQ || 
           execute_output -> icode == I_RET || 
           execute_output -> icode == I_POPQ ? 
                execute_output -> valb 
            : 0;

    e_bcond = cond_holds(cc, execute_output->ifun);

    if(execute_output->icode == I_RRMOVQ) alua = e_bcond ? execute_output->vala : execute_output->valb;
...
void do_memory_stage()
{
    ...
    mem_read = memory_output->icode == I_MRMOVQ || memory_output->icode == I_POPQ || memory_output->icode == I_RET ? true : false;

    mem_addr = (memory_output->icode) == (I_RMMOVQ) || (memory_output->icode) == (I_PUSHQ) || 
               (memory_output->icode) == (I_CALL) || (memory_output->icode) == (I_MRMOVQ) ?
                    memory_output -> vale 
                : (memory_output->icode) == (I_POPQ) || (memory_output->icode) == (I_RET) ?
                    memory_output -> vala : 0;

    mem_data = memory_output->vala;

    mem_write = (memory_output->icode) == (I_RMMOVQ) || (memory_output->icode) ==  (I_PUSHQ) || (memory_output->icode) == (I_CALL) ? true : false;

    dmem_error = false;

    if (mem_read) {
        if ((dmem_error |= !get_word_val(mem, mem_addr, &valm))) {
    ...
    writeback_input->status = dmem_error ? STAT_HLT : memory_output->status;
...
void help() {
    ...
    printf("cycle n           -  cycle n instructions\n");
